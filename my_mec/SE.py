...
def SE_choose_action(state, time_flag):     # 选择资源最充足的服务器，就可以服务最多的用户
    ...
    if resource_index:
        index = action_index[np.argmax(resource_index)] #random.choice(action_index)
...
if __name__ == '__main__':
    ...
    f = open('./log/B/100N_SE_20M.csv', 'w', encoding='utf-8', newline="")  # 这个文件用于存放实验过程中的输出情况
    ...
    for i in range(Constants.episodes):
        print("epoch:", i)
        state = env.reset()
        action = SE_choose_action(state, env.local_time)
        episode_reward = 0.
        episode_user_num = 0
        episode_cost = 0
        episode_t_exe = 0
        for j in range(max_steps):
            ...
            next_action = SE_choose_action(next_state, env.local_time)
            action = next_action
            state = next_state
    ...
    # nohup python SE.py >./log/B/100N_SE_20M.log 2>&1&