...
######## N
# ax.set_aspect(0.9) #
ax.set_aspect(52)  # P_DQN
plt.xlim(xmin=45, xmax=205)  # N
# plt.ylim(ymin=1.5, ymax=3.0) # I-PDQN
plt.ylim(ymin=2.5, ymax=4.2)  # P-DQN
plt.xlabel('用户数量', fontproperties="SimSun", fontsize=12)
plt.ylabel('平均服务时间$\mathrm{(s)}$', fontproperties="SimSun", fontsize=12)

#--F---
# ax.set_aspect(0.85)
# plt.xlim(xmin=3.8, xmax=7.2)     # F
# plt.ylim(ymin=1.5, ymax=3.5)    # I-PDQN
# ## plt.ylim(ymin=2.0, ymax=4.0)
# plt.xlabel('边缘服务器计算性能$\mathrm{(GHz)}$', fontproperties="SimSun", fontsize=12)
# plt.ylabel('平均服务时间$\mathrm{(s)}$', fontproperties="SimSun", fontsize=12)

#---B--
# # ax.set_aspect(4)  # P_DQN
# # plt.xlim(xmin=14, xmax=31)     # B P-DQN
# ax.set_aspect(5)
# plt.xlim(xmin=9, xmax=26)
# # plt.ylim(ymin=2.0, ymax=4.0)    # P-DQN
# plt.ylim(ymin=1.5, ymax=3.2)
#
# plt.xlabel('边缘服务器带宽$\mathrm{(MHz)}$', fontproperties="SimSun", fontsize=12)
# plt.ylabel('平均服务时间$\mathrm{(s)}$', fontproperties="SimSun", fontsize=12)
...
plt.savefig(r'./log/Datas59/Figs/PDQN_N_Time.svg',dpi=1200, format='svg',bbox_inches ='tight')      # 保存图片
