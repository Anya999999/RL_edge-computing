...
def largest_resource(state):
    ...
    if server_index and tolerant_delay:
        ...
        server_source = [0 for _ in range(Constants.edge_num)]
        for i in range(len(u2s_distance)):
            ...
            if trans_time[server_index[i]-1] < float("inf") and remain_capacity[server_index[i] - 1] >= min_required_resource:
                ...
                server_source[server_index[i]-1] = remain_capacity[server_index[i] - 1]
        # print("trans time:", trans_time)
        if available_server:
            index = np.argmax(server_source)+1
            param = 1
        else:
            index = 0
            param = 1
...
if __name__ == '__main__':
    ...
    f = open('./log/N/100N_glr.csv.csv', 'w', encoding='utf-8', newline="")  # 这个文件用于存放实验过程中的输出情况
    ...
    for i in range(Constants.episodes):
        ...
        for j in range(max_steps):
            ...
            next_action = largest_resource(next_state)
            action = next_action
            state = next_state
    ...
    # python largest_resources.py >./log/N/100N_glr.log 2>&1&